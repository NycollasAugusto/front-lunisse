...
export const SessaoDetalhes = () => {
    ...
    useEffect(() => {
        const loadSessionData = async () => {
            try {
                ...
                const patientData = patient.find(p => p.id === sessionData.patientId);
    ...
    const statusOption = [
        ...
        { value: 'concluído', label: 'Concluído', color: 'bg-green-100 text-dark' },
        { value: 'cancelado', label: 'Cancelado', color: 'bg-red-100 text-dark' }
    ]
    const currentStatus = statusOption.find(s.value === session.updateSessionstatus);
    return (
